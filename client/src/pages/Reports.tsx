...
const generatePDFReport = async (reportData: any, user: any) => {
  const doc = new jsPDF();
  ...
  doc.text(`Report Period: ${reportData.startDate} to ${reportData.endDate}`, 20, 56);
  ...
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text(`Total Emissions: ${reportData.totalEmissions.toFixed(2)} kg CO2e`, 20, 85);
  doc.text(`Total Entries: ${reportData.entryCount}`, 20, 91);
  doc.text(`Average Daily: ${reportData.averageDaily.toFixed(2)} kg CO2e/day`, 20, 97);
  
  // Category breakdown
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text('Category Breakdown', 20, 115);
  
  const categoryData = reportData.breakdown.map((item: any) => [
  ...
  if (finalY + 40 < pageHeight) {
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('Detailed Emissions', 20, finalY + 20);
    
    const detailedData = reportData.detailedEmissions.slice(0, 20).map((item: any) => [
    ...
    autoTable(doc, {
      startY: finalY + 25,
      head: [['Date', 'Category', 'Quantity', 'CO2 Emissions']],
      body: detailedData,
      theme: 'striped',
      headStyles: { fillColor: [16, 185, 129] },
      margin: { left: 20, right: 20 }
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(
      `Page ${i} of ${pageCount} | Generated by CarbonSense © ${new Date().getFullYear()}`,
      pageWidth / 2,
      pageHeight - 10,
      { align: 'center' }
    );
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const reportData = {
        ...
        totalEmissions,
        entryCount: filteredEmissions.length,
        averageDaily,
        breakdown,
        detailedEmissions: filteredEmissions
      };
      
      // Generate PDF or CSV based on format
      if (reportForm.format === 'pdf') {
        const pdf = await generatePDFReport(reportData, user);
        pdf.save(`carbon-report-${reportForm.reportType}-${Date.now()}.pdf`);
      } else {
        // Generate CSV
        const csvContent = [
      ...
      toast({
        title: "Success",
        description: `Report generated and downloaded as ${reportForm.format.toUpperCase()}`,
      });
