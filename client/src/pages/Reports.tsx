...
export default function Reports() {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      // Generate CSV report
      const csvContent = [
        ['Date', 'Category', 'Subcategory', 'Quantity', 'Unit', 'CO2 Emissions (kg)'],
        ...filteredEmissions.map((item: any) => [
      ...
      const summaryHeader = [
        `Carbon Emissions Report - ${reportForm.reportType}`,
        `Period: ${reportForm.startDate} to ${reportForm.endDate}`,
        `Total Emissions: ${totalEmissions.toFixed(2)} kg CO2e`,
        `Total Entries: ${filteredEmissions.length}`,
        `Average Daily: ${averageDaily.toFixed(2)} kg CO2e/day`,
        '',
        ''
      ].join('\n');
      
      const fullCsv = summaryHeader + csvContent;
      
      const blob = new Blob([fullCsv], { type: 'text/csv' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `carbon-report-${reportForm.reportType}-${Date.now()}.csv`;
      link.click();
      URL.revokeObjectURL(url);

      toast({
        title: "Success",
        description: "Report generated and downloaded as CSV",
      });
